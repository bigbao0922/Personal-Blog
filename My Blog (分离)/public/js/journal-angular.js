window.onload = () => {
    ...
    function renderArticle(data){
      let article = $(".article-list"),
          ...
          str = "",
          hot = "",
          artList7 = data.artList7,
          hotArt = data.hotArt,
          hotlen = hotArt.length,
          maxNum = artList7.length;
          if(maxNum > 0){
            artList7.forEach((v, i) => {
                str += `<li>
                      <a class='list-face'>
    ...
    layui.use(["element", "laypage", "laydate"], () => {
      ...
      laypage.render({
        ...
        limit: 5,
        groups: 5,
        curr: location.pathname.replace("/journal/angular/page", ""),
        jump(obj, f){
          $("#laypage a").each((i, v) => {
            let len = $("#laypage a").length;
            let pageValue = `/journal/angular/page${$(v).data("page")}`
            if($(v).data("page") === len){
              pageValue = `/journal/angular/page${len - 1}`;
            }else if($(v).data("page") === 0){
              pageValue = `/journal/angular`;
            };
            v.href = pageValue;
          })
        },
