window.onload = () => {
    ...
    function renderArticle(data){
      let article = $(".article-list"),
          ...
          str = "",
          hot = "",
          artList3 = data.artList3,
          hotArt = data.hotArt,
          hotlen = hotArt.length,
          maxNum = artList3.length;
          if(maxNum > 0){
            artList3.forEach((v, i) => {
                str += `<li>
                      <a class='list-face'>
            ...
            str = `<p class='addlog'>还没有小伙伴发布该类型的日志呢，赶紧<a class="layui-btn layui-bg-blue" href="/journal/article">发布一篇</a>吧 !</p>`
          }
          if(hotlen !=0){
            hotArt.forEach((v, i) => {
              hot += `<p>
    ...
    layui.use(["element", "laypage", "laydate"], () => {
      ...
      laypage.render({
        ...
        limit: 5,
        groups: 5,
        curr: location.pathname.replace("/journal/javascript/page", ""),
        jump(obj, f){
          $("#laypage a").each((i, v) => {
            let len = $("#laypage a").length;
            let pageValue = `/journal/javascript/page${$(v).data("page")}`
            if($(v).data("page") === len){
              pageValue = `/journal/javascript/page${len - 1}`;
            }else if($(v).data("page") === 0){
              pageValue = `/journal/javascript`;
            };
            v.href = pageValue;
          })
        },
