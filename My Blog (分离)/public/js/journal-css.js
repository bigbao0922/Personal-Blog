window.onload = () => {
    ...
    function renderArticle(data){
      let article = $(".article-list"),
          ...
          str = "",
          hot = "",
          artList2 = data.artList2,
          hotArt = data.hotArt,
          hotlen = hotArt.length,
          maxNum = artList2.length;
          if(maxNum > 0){
            artList2.forEach((v, i) => {
                str += `<li>
                      <a class='list-face'>
    ...
    layui.use(["element", "laypage", "laydate"], () => {
      ...
      laypage.render({
        ...
        limit: 5,
        groups: 5,
        curr: location.pathname.replace("/journal/css/page", ""),
        jump(obj, f){
          $("#laypage a").each((i, v) => {
            let len = $("#laypage a").length;
            let pageValue = `/journal/css/page${$(v).data("page")}`
            if($(v).data("page") === len){
              pageValue = `/journal/css/page${len - 1}`;
            }else if($(v).data("page") === 0){
              pageValue = `/journal/css`;
            };
            v.href = pageValue;
          })
        },
