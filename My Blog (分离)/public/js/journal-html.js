window.onload = () => {
  ...
  function renderArticle(data){
    let article = $(".article-list"),
        ...
        str = "",
        hot = "",
        artList1 = data.artList1,
        hotArt = data.hotArt,
        hotlen = hotArt.length,
        maxNum = artList1.length;
        if(maxNum > 0){
          artList1.forEach((v, i) => {
              str += `<li>
                    <a class='list-face'>
  ...
  layui.use(["element", "laypage", "laydate"], () => {
    ...
    laypage.render({
      ...
      limit: 5,
      groups: 5,
      curr: location.pathname.replace("/journal/html/page", ""),
      jump(obj, f){
        $("#laypage a").each((i, v) => {
          let len = $("#laypage a").length;
          let pageValue = `/journal/html/page${$(v).data("page")}`
          if($(v).data("page") === len){
            pageValue = `/journal/html/page${len - 1}`;
          }else if($(v).data("page") === 0){
            pageValue = `/journal/html`;
          };
          v.href = pageValue;
        })
      },
