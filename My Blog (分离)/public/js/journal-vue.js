window.onload = () => {
    ...
    function renderArticle(data){
      let article = $(".article-list"),
          ...
          str = "",
          hot = "",
          artList5 = data.artList5,
          hotArt = data.hotArt,
          hotlen = hotArt.length,
          maxNum = artList5.length;
          if(maxNum > 0){
            artList5.forEach((v, i) => {
                str += `<li>
                      <a class='list-face'>
    ...
    layui.use(["element", "laypage", "laydate"], () => {
      ...
      laypage.render({
        ...
        limit: 5,
        groups: 5,
        curr: location.pathname.replace("/journal/vue/page", ""),
        jump(obj, f){
          $("#laypage a").each((i, v) => {
            let len = $("#laypage a").length;
            let pageValue = `/journal/vue/page${$(v).data("page")}`
            if($(v).data("page") === len){
              pageValue = `/journal/vue/page${len - 1}`;
            }else if($(v).data("page") === 0){
              pageValue = `/journal/vue`;
            };
            v.href = pageValue;
          })
        },
