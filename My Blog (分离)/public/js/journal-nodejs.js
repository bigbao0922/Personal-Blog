window.onload = () => {
    ...
    function renderArticle(data){
      let article = $(".article-list"),
          ...
          str = "",
          hot = "",
          artList4 = data.artList4,
          hotArt = data.hotArt,
          hotlen = hotArt.length,
          maxNum = artList4.length;
          if(maxNum > 0){
            artList4.forEach((v, i) => {
                str += `<li>
                      <a class='list-face'>
    ...
    layui.use(["element", "laypage", "laydate"], () => {
      ...
      laypage.render({
        ...
        limit: 5,
        groups: 5,
        curr: location.pathname.replace("/journal/nodejs/page", ""),
        jump(obj, f){
          $("#laypage a").each((i, v) => {
            let len = $("#laypage a").length;
            let pageValue = `/journal/nodejs/page${$(v).data("page")}`
            if($(v).data("page") === len){
              pageValue = `/journal/nodejs/page${len - 1}`;
            }else if($(v).data("page") === 0){
              pageValue = `/journal/nodejs`;
            };
            v.href = pageValue;
          })
        },
