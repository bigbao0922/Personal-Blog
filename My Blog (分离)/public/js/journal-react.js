window.onload = () => {
    ...
    function renderArticle(data){
      let article = $(".article-list"),
          ...
          str = "",
          hot = "",
          artList6 = data.artList6,
          hotArt = data.hotArt,
          hotlen = hotArt.length,
          maxNum = artList6.length;
          if(maxNum > 0){
            artList6.forEach((v, i) => {
                str += `<li>
                      <a class='list-face'>
    ...
    layui.use(["element", "laypage", "laydate"], () => {
      ...
      laypage.render({
        ...
        limit: 5,
        groups: 5,
        curr: location.pathname.replace("/journal/react/page", ""),
        jump(obj, f){
          $("#laypage a").each((i, v) => {
            let len = $("#laypage a").length;
            let pageValue = `/journal/react/page${$(v).data("page")}`
            if($(v).data("page") === len){
              pageValue = `/journal/react/page${len - 1}`;
            }else if($(v).data("page") === 0){
              pageValue = `/journal/react`;
            };
            v.href = pageValue;
          })
        },
